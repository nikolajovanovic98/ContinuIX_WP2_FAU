...
!******************************************************************************
  ...
  REAL(KIND=dp) :: x_frac, x_factor
  REAL(KIND=dp), PARAMETER :: xmin = 2637888.93434821, xmax = 2648846.39826283
  ...
  DO t = 1, Solver % NumberOfActiveElements
    ...
    DO i = 1, n
      ...
      x_frac = (Nodes % x(i) - xmin) / (xmax - xmin)
      x_factor = 1.0_dp + 0.2_dp * x_frac
      
      IF (Zs(i) > Zs0) THEN
        VariableValues(Nn) = 0.0_dp
      ELSE
        VariableValues(Nn) = SMB0*(1.0-betaM*(Zs0-Zs(i))**SMBr) * x_factor
      END IF
    END DO
  END DO
